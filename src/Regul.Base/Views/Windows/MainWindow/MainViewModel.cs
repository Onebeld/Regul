	...
	public class MainViewModel : ViewModelBase
	{
		...
		private void InitializeMenuItems()
		{
			RegulMenuItems = new AvaloniaList<IRegulObject>
			{
				...
				{
					Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("FileMenu")) },
					Items =
					{
						new RegulMenuItem("Create", Command.Create(CreateNewFile), KeyGesture.Parse("Ctrl+N"))
						{
							KeyIcon = "NewFileIcon",
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("Create")) }
						},
						new RegulMenuItem("Open", Command.Create(OpenFile), KeyGesture.Parse("Ctrl+O"))
						{
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("Open")) }
						},
						new RegulMenuItem("Save", Command.Create(SaveFile), KeyGesture.Parse("Ctrl+S"))
						{
							...
							Bindings =
							{
								new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("Save")),
								new Binding(InputElement.IsEnabledProperty, new Avalonia.Data.Binding("SelectedTab").Converter(ObjectConverters.IsNotNull))
							}
						},
						new RegulMenuItem("SaveAs", Command.Create(SaveAs))
						{
							Bindings =
							{
								new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("SaveAs")),
								new Binding(InputElement.IsEnabledProperty, new Avalonia.Data.Binding("SelectedTab").Converter(ObjectConverters.IsNotNull))
							}
						},
						new RegulMenuItem("SaveAll", Command.Create(SaveAll))
						{
							Bindings =
							{
								new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("SaveAll")),
						...
						new RegulMenuItem("BackToHomePage", Command.Create(BackToHomePage))
						{
							Bindings =
							{
								new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("BackToTheHomePage")),
						...
						new RegulMenuItem("Settings", Command.Create(OpenSettings))
						{
							...
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("Settings")) }
						},
						new RegulMenuItem("ExitFromRegul", Command.Create(() => { WindowsManager.MainWindow.Close(); }), KeyGesture.Parse("Ctrl+Q"))
						{
							KeyIcon = "ExitIcon",
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("ExitFromRegul")) }
						}
					}
				},
				new RegulMenuItem("ToolsMenu", null)
				{
					Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("ToolsMenu")) },
					Items =
					{
						new RegulMenuItem("HashGenerator", Command.Create(OpenHashGenerator))
						{
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("HashGenerator")) }
						}
					}
				},
				new RegulMenuItem("ReferenceMenu", null)
				{
					Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("ReferenceMenu")) },
					Items =
					{
						new RegulMenuItem("CheckForUpdates", Command.Create(() => { App.CheckUpdate(true); }))
						{
							KeyIcon = "UpdateIcon",
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("CheckForUpdates")) }
						},
						...
						{
							KeyIcon = "MonitorIcon",
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("OpenLogsWindow")) }
						},
						...
						{
							KeyIcon = "RegulIcon",
							Bindings = { new Binding(MenuItem.HeaderProperty, new DynamicResourceExtension("AboutTheProgram")) }
						}
					}
				}
			};
		...
		private void InitializeModules()
		{
			...
			foreach (ModuleForUpdate moduleForUpdate in GeneralSettings.Settings.ModulesForUpdate)
			{
				...
				try
				{
					Extensions.ImprovedExtractToDirectory(moduleForUpdate.Path,
						Path.GetDirectoryName(moduleForUpdate.PathToModule), Overwrite.IfNewer);
			...
			foreach (string module in Directory.EnumerateFiles(RegulPaths.Modules, "*.dll", SearchOption.AllDirectories))
			{
				try
				{
			...
			if (isSuccessfullyUpdateModules)
				NotificationManager.Show(new Notification(App.GetResource<string>("Successful"), App.GetResource<string>("ModulesSuccessfullyUpdated"), NotificationType.Success));
		}
		...
		public void DropLoadModules(IEnumerable<string> files)
		{
			...
			foreach (string file in enumerable)
			{
				if (file.Contains(".zip"))
				{
					...
					{
						NotificationManager.Show(new Notification(App.GetResource<string>("Error"), App.GetResource<string>("FailedCopyModule"), NotificationType.Error));
						return;
					}
				}
			...
			NotificationManager.Show(new Notification(App.GetResource<string>("Successful"), App.GetResource<string>("ModulesUploaded"), NotificationType.Success));
		}
		...
		private async Task<Editor> GetEditor(string file)
		{
			...
			else (editor, alwaysOpenWithAnEditor) = await SelectingEditor.GetEditor(Path.GetFileName(file));
		...
		public void CreateTab(string path, Editor ed, Project project)
		{
			foreach (PleasantTabItem pleasantTabItem in TabItems)
			{
				...
				{
					NotificationManager.Show(new Notification(App.GetResource<string>("Information"), App.GetResource<string>("FileIsAlreadyOpen"), NotificationType.Information));
		...
		private void OpenAbout() => WindowsManager.CreateModalWindow<About>(WindowsManager.MainWindow);

		private void OpenLogsWindow() => WindowsManager.CreateWindow<LogsWindow>();

		public void BackToHomePage() => Page = new WelcomePage();

		public void GoToEditors() => Page = new EditorsPage();

		private void OpenHashGenerator() => WindowsManager.CreateWindow<HashGenerator>();
