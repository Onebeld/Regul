...
public static class ModuleManagerExtensions
{
    ...
    {
        if (module.Source.Languages.Length == 0 ||
        ...
        try
        {
            ...
            using (WebClient webClient = new())
            {
                file = webClient.DownloadString(
                    new Uri(module.Source.LinkForCheckUpdates ?? throw new InvalidOperationException()));
            }
            ...
            {
                string myString = lines[i] ?? throw new NullReferenceException();

                switch (lines.IndexOf(myString))
                {
                    case 0:
