...
namespace Regul.S3PI.Extensions
{
    ...
    public class ExtList : Dictionary<string, List<string>>
    {
        ...
        /// The collection is read-only.
        /// </summary>
        public static ExtList Ext => E;

        ExtList()
        {
            using (StringReader sr = new StringReader(Properties.Resources.Extensions))
            {
                string s;
                while ((s = sr.ReadLine()) != null)
                {
                    if (s.StartsWith(";")) continue;
                    List<string> t = new List<string>(s.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries));
        ...
        public List<string> this[uint key] => this["0x" + key.ToString("X8")];
        ...
        {
            get => ContainsKey(key) ? base[key] : base["*"];
