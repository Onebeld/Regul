...
namespace Regul.S3PI.Resources.RCOLChunks
{
    public class MATD : ARCOLBlock
    {
        ...
        #region Constructors
        public MATD(EventHandler handler) : base(handler, null) { }
        public MATD(EventHandler handler, Stream s) : base(handler, s) { }
        public MATD(EventHandler handler, MATD basis)
            : base(handler, null)
        {
            ...
            if (version < 0x00000103)
            {
                this.mtrl = basis.mtrl != null
                    ? new MTRL(OnRCOLChanged, basis.mtrl)
            ...
            else
            {
                ...
                this.mtnf = basis.mtnf != null
                    ? new MTNF(OnRCOLChanged, basis.mtnf)
                    : new MTNF(OnRCOLChanged);
            }
        }
        ...
        public MATD(EventHandler handler,
            uint version, uint materialNameHash, ShaderType shader, bool isVideoSurface, bool isPaintingSurface,
            MTNF mtnf)
            : base(handler, null)
        {
            this.version = version;
        ...
        public override Stream UnParse()
        {
            ...
            {
                pos = ms.Position;
                if (mtrl == null) mtrl = new MTRL(OnRCOLChanged);
                mtrl.UnParse(ms);
            }
            else
            {
                ...
                pos = ms.Position;
                if (mtnf == null) mtnf = new MTNF(OnRCOLChanged);
                mtnf.UnParse(ms);
            }
        ...
        public class MTRL : AHandlerElement, IEquatable<MTRL>, IResource
        {
            ...
            public MTRL(EventHandler handler, MTRL basis)
                : base(handler)
            {
            ...
            public MTRL(EventHandler handler, Stream s) : base(handler) { Parse(s); }
            public MTRL(EventHandler handler) : base(handler) { }
        ...
        public MTRL Mtrl
        {
            ...
            set { if (version >= 0x00000103) throw new InvalidOperationException(); if (mtrl != value) { mtrl = new MTRL(handler, mtrl); OnRCOLChanged(this, EventArgs.Empty); } }
        }
        [ElementPriority(15)]
        ...
        public MTNF Mtnf
        {
            ...
            set { if (version < 0x00000103) throw new InvalidOperationException(); if (mtnf != value) { mtnf = new MTNF(handler, mtnf) { RCOLTag = "MATD", }; OnRCOLChanged(this, EventArgs.Empty); } }
        }
