...
namespace Regul.S3PI.Resources.CatalogResource
{
    public class ObjectCatalogResource : CatalogResourceTGIBlockList
    {
        ...
        #region Constructors
        public ObjectCatalogResource(Stream s) : base(s) { }
        public ObjectCatalogResource(Stream unused, ObjectCatalogResource basis)
        ...
        public ObjectCatalogResource(
            uint version, IEnumerable<Material> materialList,
            string instanceName, //Version>=0x16
            Common common, uint objkIndex, ObjectType objectTypeFlags,
            ObjectTypeExt objectTypeFlags2, //Version>=0x1a
            WallPlacement wallPlacementFlags, Movement movementFlags, uint cutoutTilesPerLevel, uint levels,
            IEnumerable<WallCutout> mtDoorList,
            ...
            float blueprintIconOffsetMinX, float blueprintIconOffsetMinZ, float blueprintIconOffsetMaxX,
            float blueprintIconOffsetMaxZ, //Version>=0x1f
            SlotPlacement slotPlacementFlags, string surfaceType, string sourceMaterial, Moodlet moodletGiven,
            int moodletScore,
            uint unknown21, TopicRating[] topicRatings, uint fallbackIndex,
            uint modularArchEndEastVPXYIndex, uint modularArchEndWestVPXYIndex, uint modularArchConnectVPXYIndex,
            uint modularArchSingleVPXYIndex, //Version>=0x22
        ...
        protected override void Parse(Stream s)
        {
            ...
            for (int i = 0; i < topicRatings.Length; i++)
                topicRatings[i] = new TopicRating(OnResourceChanged, s);
            ...
            if (this.GetType().Equals(typeof(ObjectCatalogResource)) && s.Position != s.Length)
        ...
        protected override Stream UnParse()
        {
            ...
            for (int i = 0; i < topicRatings.Length; i++)
            {
                if (topicRatings[i] == null) topicRatings[i] = new TopicRating(OnResourceChanged);
        ...
        public class TopicRating : AHandlerElement, IEquatable<TopicRating>, IComparable<TopicRating>
        {
            ...
            #region Constructors
            public TopicRating(EventHandler handler) : base(handler) { }
            public TopicRating(EventHandler handler, Stream s) : base(handler) { Parse(s); }
            public TopicRating(EventHandler handler, TopicRating basis)
                : this(handler, basis.topic, basis.rating) { }
            public TopicRating(EventHandler handler, TopicCategory topic, int rating)
                : base(handler)
            {
                this.topic = topic;
                this.rating = rating;
        ...
        public class WallCutout : AHandlerElement, IEquatable<WallCutout>
        {
            ...
            #region Constructors
            public WallCutout(EventHandler handler, DependentList<TGIBlock> ParentTGIBlocks = null)
                : base(handler) { this.ParentTGIBlocks = ParentTGIBlocks; }
            public WallCutout(EventHandler handler, Stream s, DependentList<TGIBlock> ParentTGIBlocks = null)
                : this(handler, ParentTGIBlocks) { Parse(s); }
            public WallCutout(EventHandler handler, WallCutout basis, DependentList<TGIBlock> ParentTGIBlocks = null)
                : this(handler, basis.leftX, basis.leftZ, basis.rightX, basis.rightZ, basis.levelOffset, basis.wallMaskIndex, ParentTGIBlocks ?? basis.ParentTGIBlocks) { }
            public WallCutout(EventHandler handler, float leftX, float leftZ, float rightX, float rightZ,
                uint levelOffset, uint wallMaskIndex, DependentList<TGIBlock> ParentTGIBlocks = null)
                : this(handler, ParentTGIBlocks)
            {
                this.leftX = leftX;
                this.leftZ = leftZ;
                this.rightX = rightX;
        ...
        public class WallCutoutList : DependentList<WallCutout>
        {
            ...
            public DependentList<TGIBlock> ParentTGIBlocks
            {
                ...
                set { if (_ParentTGIBlocks != value) { _ParentTGIBlocks = value; foreach (WallCutout i in this) i.ParentTGIBlocks = _ParentTGIBlocks; } }
            }
            ...
            public WallCutoutList(EventHandler handler, IEnumerable<WallCutout> mtDoorList, DependentList<TGIBlock> ParentTGIBlocks = null)
                : this(null, ParentTGIBlocks) { elementHandler = handler; foreach (WallCutout t in mtDoorList) this.Add((WallCutout)t.Clone(null)); this.handler = handler; }
            #endregion

            #region Data I/O
            protected override int ReadCount(Stream s) { return (new BinaryReader(s)).ReadByte(); }
            protected override WallCutout CreateElement(Stream s) { return new WallCutout(elementHandler, s); }
            ...
            public override void Add() { this.Add(new WallCutout(handler, _ParentTGIBlocks)); }
        ...
        public TopicRating[] TopicRatings
        {
            ...
            set
            {
                ...
                if (!topicRatings.Equals(value)) { topicRatings = value == null ? null : (TopicRating[])value.Clone(); OnResourceChanged(this, EventArgs.Empty); }
            }
        }
