...
namespace Regul.S3PI.Resources.MeshChunks
{
    ...
    public class MODL : ARCOLBlock
    {
        public class BoundingBoxList : DependentList<BoundingBox>
        {
            ...
            protected override BoundingBox CreateElement(Stream s) { return new BoundingBox(handler, s); }
        ...
        public class LODEntryList : DependentList<LODEntry>
        {
            ...
            {
                foreach (LODEntry element in this)
                {
                    WriteElement(s, element);
                }
            }

            protected override LODEntry CreateElement(Stream s) { return new LODEntry(handler, s); }
        ...
        {
            private GenericRCOLResource.GenericRCOLResource.ChunkReference mModelLodIndex;
            private LODInfoFlags mFlags;
            private LODId mId;
            private float mMinZValue;
            private float mMaxZValue;

            public LODEntry(EventHandler handler) : this(handler, new GenericRCOLResource.GenericRCOLResource.ChunkReference(handler, 0), (LODInfoFlags)0, LODId.LowDetail, float.MinValue, float.MaxValue) { }
            public LODEntry(EventHandler handler, LODEntry basis) : this(handler, basis.ModelLodIndex, basis.Flags, basis.Id, basis.MinZValue, basis.MaxZValue) { }
            public LODEntry(EventHandler handler, Stream s) : base(handler) { Parse(s); }
            ...
            [ElementPriority(1)]
            public GenericRCOLResource.GenericRCOLResource.ChunkReference ModelLodIndex
            {
                get { return mModelLodIndex; }
                set { if (mModelLodIndex != value) { mModelLodIndex = new GenericRCOLResource.GenericRCOLResource.ChunkReference(handler, value); OnElementChanged(); } }
            }
            [ElementPriority(2)]
        ...
        public MODL(EventHandler handler) : this(handler, 256, new BoundingBox(handler), new BoundingBoxList(handler), 0, 0f, new LODEntryList(handler)) { }
        public MODL(EventHandler handler, MODL basis) : this(handler, basis.Version, new BoundingBox(handler, basis.Bounds), new BoundingBoxList(handler, basis.ExtraBounds),basis.FadeType,basis.CustomFadeDistance, new LODEntryList(handler, basis.Entries)) { }
        public MODL(EventHandler handler, Stream s) : base(handler, s) { }
        public MODL(EventHandler handler, uint version, BoundingBox bounds, BoundingBoxList extraBounds, uint fadeType,
            float customFadeDistance, LODEntryList entries)
            : base(handler, null)
        {
            mVersion = version;
            mBounds = bounds;
        ...
        public override Stream UnParse()
        {
            ...
            if (mBounds == null) mBounds = new BoundingBox(handler);
