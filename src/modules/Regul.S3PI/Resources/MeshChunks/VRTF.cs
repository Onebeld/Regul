...
namespace Regul.S3PI.Resources.MeshChunks
{
    public class VRTF : ARCOLBlock
    {
        ...
        {
            public Default(EventHandler handler)
                : base(handler)
            {
            }
        ...
        {
            VRTF v = new Default(null);
            v.Layouts.Add(new ElementLayout(null, ElementFormat.Short4, 0, ElementUsage.Position, 0));
        ...
        {
            VRTF v = new Default(null);
            v.Layouts.Add(new ElementLayout(null, ElementFormat.UShort4N, 0, ElementUsage.Position, 0));
            v.Layouts.Add(new ElementLayout(null, ElementFormat.Short4_DropShadow, (byte)v.Layouts.Select(x => VRTF.ByteSizeFromFormat(x.Format)).Sum(), ElementUsage.UV, 0));
        ...
        public class VertexElementLayoutList : DependentList<ElementLayout>
        {
            ...
            {
                return new ElementLayout(handler, s);
            }
        ...
        public class ElementLayout : AHandlerElement, IEquatable<ElementLayout>
        {
            ...
            public ElementLayout(EventHandler handler): base(handler){}
            public ElementLayout(EventHandler handler, ElementLayout basis): this(handler,basis.Format,basis.Offset,basis.Usage,basis.UsageIndex){}
            public ElementLayout(EventHandler handler, Stream s): base(handler){Parse(s);}
            public ElementLayout(EventHandler handler, ElementFormat format, byte offset, ElementUsage usage,
                byte usageIndex) : base(handler)
            {
                mFormat = format;
                mOffset = offset;
                mUsage = usage;
        ...
        public VRTF(EventHandler handler)
            : base(handler, null)
        {
            mVersion = 0x00000002;
        }
        public VRTF(EventHandler handler, VRTF basis): this(handler,basis.Version,basis.Stride, basis.Layouts, basis.ExtendedFormat){}
        public VRTF(EventHandler handler, Stream s): base(handler, s){}
        public VRTF(EventHandler handler, uint version, int stride, VertexElementLayoutList layouts,
            bool extendedFormat) : base(handler, null)
        {
