...
namespace Regul.S3PI.Resources.CASPartResource
{
    public class BoneResource : AResource
    {
        ...
        public BoneResource(Stream s) : base(s) { if (stream == null) { stream = UnParse(); OnResourceChanged(this, EventArgs.Empty); } stream.Position = 0; Parse(stream); }
        ...
        private void Parse(Stream s)
        {
            ...
            for (int l = 0; l < matrices.Length; l++)
                matrices[l] = new Matrix4x3(null, s);

            bones = new BoneList(OnResourceChanged, names.Zip(matrices, (name, matrix) => new Bone(null, name, matrix)));
        }
        ...
        public class MatrixRow : AHandlerElement, IEquatable<MatrixRow>
        {
            ...
            #region Constructors
            public MatrixRow(EventHandler handler) : base(handler) { }
            public MatrixRow(EventHandler handler, Stream s) : base(handler) { Parse(s); }
            public MatrixRow(EventHandler handler, MatrixRow basis) : this(handler, basis.x, basis.y, basis.z) { }
            public MatrixRow(EventHandler handler, float x, float y, float z) : base(handler) { this.x = x; this.y = y; this.z = z; }
            #endregion
        ...
        public class Matrix4x3 : AHandlerElement, IEquatable<Matrix4x3>
        {
            ...
            #region Constructors
            public Matrix4x3(EventHandler handler) : base(handler) { }
            public Matrix4x3(EventHandler handler, Stream s) : base(handler) { Parse(s); }
            public Matrix4x3(EventHandler handler, Matrix4x3 basis) : this(handler, basis.right, basis.up, basis.back, basis.translate) { }
            ...
            #region Content Fields
            [ElementPriority(1)]
            public MatrixRow Right { get { return right; } set { if (!right.Equals(value)) { right =new MatrixRow(handler, value); OnElementChanged(); } } }
            [ElementPriority(2)]
            public MatrixRow Up { get { return up; } set { if (!up.Equals(value)) { up = new MatrixRow(handler, value); OnElementChanged(); } } }
            [ElementPriority(3)]
            public MatrixRow Back { get { return back; } set { if (!back.Equals(value)) { back = new MatrixRow(handler, value); OnElementChanged(); } } }
            [ElementPriority(4)]
            public MatrixRow Translate { get { return translate; } set { if (!translate.Equals(value)) { translate = new MatrixRow(handler, value); OnElementChanged(); } } }
        ...
        public class Bone : AHandlerElement, IEquatable<Bone>
        {
            ...
            #region Constructors
            public Bone(EventHandler handler) : base(handler) { }
            public Bone(EventHandler handler, Bone basis) : this(handler, basis.name, basis.inverseBindPose) { }
            public Bone(EventHandler handler, string name, Matrix4x3 inverseBindPose) : base(handler) { this.name = name; this.inverseBindPose = new Matrix4x3(handler, inverseBindPose); }
            #endregion
            ...
            [ElementPriority(2)]
            public Matrix4x3 InverseBindPose { get { return inverseBindPose; } set { if (!inverseBindPose.Equals(value)) { inverseBindPose = new Matrix4x3(handler, value); OnElementChanged(); } } }
