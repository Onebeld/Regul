...
public partial class ColorSpectrum : TemplatedControl
{
    ...
    private List<Hsv> _hsvValues = new();
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        if (_inputTarget != null)
        {
            ...
            _inputTarget.PointerExited += InputTarget_PointerLeave;
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        IncrementDirection direction =
            (incrementComponent == HsvComponent.Hue && key is Key.Left or Key.Up) ||
            (incrementComponent != HsvComponent.Hue && key is Key.Right or Key.Down)
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        if (change.Property == ColorProperty)
        {
            ...
            if (!_updatingColor)
            {
                ...
                const double div = 1.0 / 255.0;
                HsvColor = newHsv.ToHsvColor(color.A * div);
    ...
    public void RaiseColorChanged()
    {
        ...
        {
            ColorChangedEventArgs colorChangedEventArgs = new(_oldColor, newColor);
    ...
    private void UpdateEllipse()
    {
        ...
        hsvColor.S = MathUtilities.Clamp(hsvColor.S, _minSaturationFromLastBitmapCreation * 0.01, _maxSaturationFromLastBitmapCreation * 0.01);
        hsvColor.V = MathUtilities.Clamp(hsvColor.V, _minValueFromLastBitmapCreation * 0.01, _maxValueFromLastBitmapCreation * 0.01);
        ...
        if (_shapeFromLastBitmapCreation == ColorSpectrumShape.Box)
        {
            ...
            if (_componentsFromLastBitmapCreation is ColorSpectrumComponents.HueSaturation or ColorSpectrumComponents.SaturationHue)
            {
                sPercent = 1 - sPercent;
            }
            else
        ...
        // Remember the bitmap size follows physical device pixels
        double scale = LayoutHelper.GetLayoutScale(this);
        Canvas.SetLeft(_selectionEllipsePanel, xPosition / scale - _selectionEllipsePanel.Width * 0.5);
        Canvas.SetTop(_selectionEllipsePanel, yPosition / scale - _selectionEllipsePanel.Height * 0.5);

        // We only want to bother with the color name tool tip if we can provide color names.
        if (IsFocused && ColorHelper.ToDisplayNameExists)
            ToolTip.SetIsOpen(_selectionEllipsePanel, true);
    ...
    private async void CreateBitmapsAndColorMap()
    {
        ...
        List<byte> bgraMinPixelData = new();
        List<byte> bgraMiddle1PixelData = new();
        List<byte> bgraMiddle2PixelData = new();
        List<byte> bgraMiddle3PixelData = new();
        List<byte> bgraMiddle4PixelData = new();
        List<byte> bgraMaxPixelData = new();
        List<Hsv> newHsvValues = new();
        ...
        int pixelCount = pixelDimension * pixelDimension;
        int pixelDataSize = pixelCount * 4;
        bgraMinPixelData.Capacity = pixelDataSize;

        // We'll only save pixel data for the middle bitmaps if our third dimension is hue.
        if (components is ColorSpectrumComponents.ValueSaturation or ColorSpectrumComponents.SaturationValue)
        {
        ...
        await Task.Run(() =>
        {
            ...
            if (shape == ColorSpectrumShape.Box)
            {
                for (int x = pixelDimension - 1; x >= 0; --x)
                {
                    ...
                    {
                        FillPixelForBox(
                            x, y, hsv, pixelDimension, components, minHue, maxHue, minSaturation, maxSaturation,
                            minValue, maxValue,
                            bgraMinPixelData, bgraMiddle1PixelData, bgraMiddle2PixelData, bgraMiddle3PixelData,
                            bgraMiddle4PixelData, bgraMaxPixelData,
                            newHsvValues);
                    }
                }
            }
            else
            {
                for (int y = 0; y < pixelDimension; ++y)
                {
                    ...
                    {
                        FillPixelForRing(
                            x, y, pixelDimension * 0.5, hsv, components, minHue, maxHue, minSaturation, maxSaturation,
                            minValue, maxValue,
                            bgraMinPixelData, bgraMiddle1PixelData, bgraMiddle2PixelData, bgraMiddle3PixelData,
                            bgraMiddle4PixelData, bgraMaxPixelData,
                            newHsvValues);
        ...
        List<Hsv> newHsvValues)
    {
        double hMin = minHue;
        double hMax = maxHue;
        double sMin = minSaturation * 0.01;
        double sMax = maxSaturation * 0.01;
        double vMin = minValue * 0.01;
        double vMax = maxValue * 0.01;
        ...
        {
            case ColorSpectrumComponents.HueValue:
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + yPercent * (hMax - hMin);
                hsvMin.V = hsvMiddle1.V =
                    hsvMiddle2.V = hsvMiddle3.V = hsvMiddle4.V = hsvMax.V = vMin + xPercent * (vMax - vMin);
                hsvMin.S = 0;
                hsvMax.S = 1;
                break;
            ...
            case ColorSpectrumComponents.HueSaturation:
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + yPercent * (hMax - hMin);
                hsvMin.S = hsvMiddle1.S =
                    hsvMiddle2.S = hsvMiddle3.S = hsvMiddle4.S = hsvMax.S = sMin + xPercent * (sMax - sMin);
                hsvMin.V = 0;
                hsvMax.V = 1;
                break;
            ...
            case ColorSpectrumComponents.ValueHue:
                hsvMin.V = hsvMiddle1.V =
                    hsvMiddle2.V = hsvMiddle3.V = hsvMiddle4.V = hsvMax.V = vMin + yPercent * (vMax - vMin);
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + xPercent * (hMax - hMin);
                hsvMin.S = 0;
                hsvMax.S = 1;
                break;
            ...
            case ColorSpectrumComponents.SaturationHue:
                hsvMin.S = hsvMiddle1.S =
                    hsvMiddle2.S = hsvMiddle3.S = hsvMiddle4.S = hsvMax.S = sMin + yPercent * (sMax - sMin);
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + xPercent * (hMax - hMin);
                hsvMin.V = 0;
                hsvMax.V = 1;
        ...
        if (components is ColorSpectrumComponents.HueSaturation or ColorSpectrumComponents.SaturationHue)
        {
        ...
        List<Hsv> newHsvValues)
    {
        ...
        double theta = Math.Atan2(radius - yToUse, radius - xToUse) * InvRad;
        theta += 180.0;
        ...
        const double div = 1.0 / 360;
        double thetaPercent = theta * div;

        switch (components)
        {
            case ColorSpectrumComponents.HueValue:
                hsvMin.H = hsvMiddle1.H = hsvMiddle2.H =
                    hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + thetaPercent * (hMax - hMin);
                hsvMin.V = hsvMiddle1.V =
                    hsvMiddle2.V = hsvMiddle3.V = hsvMiddle4.V = hsvMax.V = vMin + r * (vMax - vMin);
                hsvMin.S = 0;
                hsvMax.S = 1;
                break;
            ...
            case ColorSpectrumComponents.HueSaturation:
                hsvMin.H = hsvMiddle1.H = hsvMiddle2.H =
                    hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + thetaPercent * (hMax - hMin);
                hsvMin.S = hsvMiddle1.S =
                    hsvMiddle2.S = hsvMiddle3.S = hsvMiddle4.S = hsvMax.S = sMin + r * (sMax - sMin);
                hsvMin.V = 0;
                hsvMax.V = 1;
                break;

            case ColorSpectrumComponents.ValueHue:
                hsvMin.V = hsvMiddle1.V = hsvMiddle2.V =
                    hsvMiddle3.V = hsvMiddle4.V = hsvMax.V = vMin + thetaPercent * (vMax - vMin);
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + r * (hMax - hMin);
                hsvMin.S = 0;
                hsvMax.S = 1;
                break;
            ...
            case ColorSpectrumComponents.SaturationHue:
                hsvMin.S = hsvMiddle1.S = hsvMiddle2.S =
                    hsvMiddle3.S = hsvMiddle4.S = hsvMax.S = sMin + thetaPercent * (sMax - sMin);
                hsvMin.H = hsvMiddle1.H =
                    hsvMiddle2.H = hsvMiddle3.H = hsvMiddle4.H = hsvMax.H = hMin + r * (hMax - hMin);
                hsvMin.V = 0;
                hsvMax.V = 1;
    ...
    private void UpdateBitmapSources()
    {
        ...
        switch (components)
        {
            ...
            case ColorSpectrumComponents.SaturationValue:
            {
                ...
                const double div = 1.0 / 60.0;
                double sextant = hsvColor.H * div;
    ...
    private bool SelectionEllipseShouldBeLight()
    {
        ...
        double lum = ColorHelper.GetRelativeLuminance(displayedColor);
