...
{
    public static readonly HueConverter Instance = new();
...
{
    public static readonly AlphaConverter Instance = new();
...
public class HsvProperties : ColorPickerProperties
{
    ...
    {
        if (hue is < 0.0 or > 360.0) return false;
    ...
    {
        if (saturation is < 0.0 or > 100.0) return false;
    ...
    {
        if (value is < 0.0 or > 100.0) return false;
...
public class RgbProperties : ColorPickerProperties
{
    ...
    public override void UpdatePropertyValues()
    {
        ...
        {
            _updating = true;
            HSV hsv = new(ColorPicker.Value1, ColorPicker.Value2, ColorPicker.Value3);
            RGB rgb = hsv.ToRGB();
            Red = (byte)rgb.R;
            Green = (byte)rgb.G;
            Blue = (byte)rgb.B;
...
public class CmykProperties : ColorPickerProperties
{
    ...
    {
        if (cyan is < 0.0 or > 100.0) return false;
    ...
    {
        if (magenta is < 0.0 or > 100.0) return false;
    ...
    {
        if (yellow is < 0.0 or > 100.0) return false;
    ...
    {
        if (blackKey is < 0.0 or > 100.0) return false;
    ...
    public override void UpdateColorPickerValues()
    {
        ...
        {
            _updating = true;
            CMYK cmyk = new(Cyan, Magenta, Yellow, BlackKey);
            HSV hsv = cmyk.ToHSV();
            ColorPicker.Value1 = hsv.H;
            ColorPicker.Value2 = hsv.S;
            ColorPicker.Value3 = hsv.V;
...
public class AlphaProperties : ColorPickerProperties
{
    ...
    {
        if (alpha is < 0.0 or > 100.0) return false;
...
public class ColorPicker : TemplatedControl
{
    ...
    {
        RoutedEventArgs e = new(ChangeColorEvent);
        RaiseEvent(e);
