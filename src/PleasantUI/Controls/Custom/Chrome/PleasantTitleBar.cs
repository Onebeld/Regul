...
public class PleasantTitleBar : TemplatedControl
{
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        if (VisualRoot is PleasantWindow window)
        {
            ...
            _dragWindow.DoubleTapped += (_, _) =>
            {
                if (window.WindowButtons is WindowButtons.CloseAndExpand or WindowButtons.All)
                    window.WindowState = window.WindowState == WindowState.Maximized
                        ? WindowState.Normal
                        : WindowState.Maximized;
            };
            window.PointerReleased += (_, _) => _contextMenu.Close();
        }
        ...
        if (VisualRoot is PleasantWindow window)
            _disposables = new CompositeDisposable
            {
                ...
                window.GetObservable(Window.IconProperty)
                    .Subscribe(_ => _image.Source = window.Icon.ToBitmap()),
                ...
                window.GetObservable(PleasantWindow.ShowCustomTitleBarProperty).Subscribe(x =>
                {
                    ...
                    else
                    {
                        ...
                        if (window.ShowMenuBar == false) IsVisible = false;
                    }
                })
            };
    }
