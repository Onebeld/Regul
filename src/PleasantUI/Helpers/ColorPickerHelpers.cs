...
internal static class ColorPickerHelpers
{
        ...
        bool isSaturationValueMaxForced)
    {
        ...
        byte[] bitmap = await Task.Run<byte[]>(() =>
        {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        if (y == 0)
                        {
                            ...
                            bgraPixelData[pixelDataIndex + 0] = Convert.ToByte(rgbColor.B * rgbColor.A * div);
                            bgraPixelData[pixelDataIndex + 1] = Convert.ToByte(rgbColor.G * rgbColor.A * div);
                            bgraPixelData[pixelDataIndex + 2] = Convert.ToByte(rgbColor.R * rgbColor.A * div);
        ...
        double maxBound)
    {
        ...
        string originalColorName = ColorHelper.ToDisplayName(originalHsv.ToColor());
        ...
        while (newColorName == originalColorName)
        {
            ...
            if (newValue > maxBound)
            {
                ...
                else
                {
                    ...
                    newColorName = ColorHelper.ToDisplayName(newHsv.ToColor());
                    break;
                }
            }
        ...
        if (shouldFindMidPoint)
        {
            ...
            while (newColorName == currentColorName)
            {
                ...
                currentColorName = ColorHelper.ToDisplayName(currentHsv.ToColor());
            }

            newValue = (startValue + currentValue + startEndOffset) * 0.5;
